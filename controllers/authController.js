...
exports.protect = catchAsync(async (req, res, next) => {
  // 1. Get the token a check if it exists
  let token;
  if (
  ...
  // 4. Check if the user changed password agfrer the token was issued
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      // 3. Check if the user changed password agfrer the token was issued
      if (currentUser.changedPasswordAfter(decoded.iat)) {
        return next();
      }
...
exports.restrictTo =
  ...
  async (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError('You are not authorized to preform this action', 403)
      );
    }
    next();
  };
...
exports.restrictReview = () => async (req, res, next) => {
  const tour = await Tour.findOne({ _id: req.body.tour }).populate('bookings');
  if (
    !tour.bookings.find((booking) => booking.user.toString() === req.body.user)
  ) {
    next(
      new AppError(
        'You must book this Tour to write a review on this Tour',
        401
      )
    );
  }
